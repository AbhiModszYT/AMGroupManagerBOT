...
from Am import INSPECTOR as SUDO_USERS
from Am import pbot
from Am.Database import f_sub_sql as sql
...
def _onUnMuteRequest(client, cb):
    ...
    if chat_db:
        ...
        if chat_member.restricted_by:
            ...
            if (
                not client.get_chat_member(
                    chat_id, (client.get_me()).id).status
                == "administrator"
            ):
                client.send_message(
                    chat_id,
...
def config(client, message):
    ...
    if user.status == "creator" and user.user.id in SUDO_USERS:
        chat_id = message.chat.id
...
__help__ = """
*Force Subscribe:*
• Am can mute members who are not subscribed your channel until they subscribe
