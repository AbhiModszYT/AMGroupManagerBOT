...
import Am.Database.locks_sql as sql
from Am import dispatcher, INSPECTOR, LOGGER
...
from Am.Plugins.Admin.connection import connected
from Am.Database.approve_sql import is_approved
        ...
        if len(args) >= 1:
            ...
            if ltype in LOCK_TYPES:
                ...
                if conn:
                    ...
                    chat_id = conn
                    chat_name = chat.title
                    text = f"Unlocked {ltype} for everAm in {chat_name}!"
                else:
                    if update.effective_message.chat.type == "private":
                        send_message(
                            update.effective_message,
                    ...
                    chat_id = update.effective_chat.id
                    chat_name = update.effective_message.chat.title
                    text = f"Unlocked {ltype} for everAm!"
...
@Ammsg((Filters.all & Filters.chat_type.groups), group=PERM_GROUP)
@user_not_admin
...
LOCK_HANDLER = CommandHandler("lock", lock, pass_args=True, run_async=True)  # , filters=Filters.chat_type.group)
UNLOCK_HANDLER = CommandHandler(
    "unlock", unlock, pass_args=True, run_async=True
)  # , filters=Filters.chat_type.group)
LOCKED_HANDLER = CommandHandler("locks", list_locks, run_async=True)  # , filters=Filters.chat_type.group)
