...
import Am.Database.feds_sql as sql
from Am import (
    EVENT_LOGS,
    LOGGER,
    SUPPORT_CHAT,
...
from Am.Handlers.alternate import send_message
...
from Am.Handlers.string_handling import markdown_parser
...
def leave_fed(update: Update, context: CallbackContext):
    ...
    if getuser in "creator" or user.id in INSPECTOR:
...
def user_join_fed(update: Update, context: CallbackContext):
    ...
    if is_user_fed_owner(fed_id, user.id) or user.id in INSPECTOR:
...
def user_transfer_fed(update: Update, context: CallbackContext):
    ...
    args = context.args
    chat = update.effective_chat 
    user = anony = update.effective_user  
    msg = update.effective_message  
    fed_id = sql.get_fed_id(chat.id)
...
def fed_info(update: Update, context: CallbackContext):
    ...
    keyboard = InlineKeyboardMarkup([[
            InlineKeyboardButton(
                "🔱 Fed Admins", callback_data="fed_admin({})".format(fed_id))
        ]])


    update.effective_message.reply_text(text,reply_markup=keyboard, parse_mode=ParseMode.HTML)
...
def button(update: Update, context: CallbackContext) -> str:
    ...
    match = re.match(r"fed_admin\((.+?)\)", query.data)
...
def fed_ban(update: Update, context: CallbackContext):
    ...
    if int(user_id) in INSPECTOR:
        message.reply_text("INSPECTOR cannot be fed banned!")
        return

    if int(user_id) in REQUESTER:
        message.reply_text("REQUESTER cannot be fed banned!")
        return
            ...
            with BytesIO() as file:
                ...
                for x in splitting:
                    ...
                    if str(import_userid) == str(OWNER_ID):
                        failed += 1
                        continue
                    if int(import_userid) in INSPECTOR:
                        failed += 1
                        continue
                    if int(import_userid) in REQUESTER:
                        failed += 1
                        continue
...
FED_USER_HELP = """*🎩 Any user:*
 • `/fbanstat`*:* Shows if you/or the user you are replying to or their username is fbanned somewhere or not
 • `/fednotif <on/off>`*:* Federation settings not in PM when there are users who are fbaned/unfbanned
 • `/frules`*:* See Federation regulations\n"""

__help__ = [ """Everything is fun, until a spammer starts entering your group, and you have to block it. Then you need to start banning more, and more, and it hurts.
...
• `/feduserhelp`*:* Provides help for commands anAm can use""",
...
FEDSTAT_USER = DisableAbleCommandHandler(["fstat", "fbanstat"], fed_stat_user, run_async=True)
...
FEDADMIN_QUERY_HANDLER = CallbackQueryHandler(button, pattern=r"fed_admin", run_async=True)
...
dispatcher.add_handler(FEDADMIN_QUERY_HANDLER)