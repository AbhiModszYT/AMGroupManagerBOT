...
from Am.__help__ import STATS, USER_INFO
import Am.Database.userinfo_sql as sql
from Am.Plugins.disable import DisableAbleCommandHandler
from Am.Database.global_bans_sql import is_user_gbanned
from Am.Database.afk_sql import is_afk, check_afk_status
from Am.Database.users_sql import get_user_num_chats
from Am.Handlers.validation import sudo_plus, user_admin, support_plus
...
def group_info(update: Update, context: CallbackContext):
    ...
    if args and len(args) >= 1:
            ...
            msg = f"<b>Group info of</b> - <code>{entity.title}</code>\n" 
            msg += f"\n<b>ID:</b> <code>{entity.id}</code>"
            msg += f"\n<b>Title:</b> <code>{entity.title}</code>"
            msg += f"\n<b>Description:</b> <code>{entity.description}</code>"
            msg += f"\n<b>Supergroup:</b> <code>{entity.type}</code>"
            msg += f"\n<b>Can Send:</b> <code>{entity.permissions.can_send_messages}</code>"
            msg += f"\n<b>Bio:</b> <code>{entity.bio}</code>"
            msg += f"\n<b>Slowmode:</b> <code>{entity.slow_mode_delay}</code>"
            msg += f"\n<b>Location:</b> <code>{entity.location}</code>"
            if entity.username:
                msg += f"\n<b>Username:</b> @{entity.username}"
            msg += "\n\n<b>Member Stats:</b>"
            msg += f"\n<code>Admins:</code> <code>{len(totaladmin)}</code>"
            if entity.invite_link:
                msg += f"\n<b>Link:</b> {entity.invite_link}"
...
def info(update: Update, context: CallbackContext):
    ...
    if user_id not in [bot.id, 777000, 1087968824]:
        ...
        text += f"\n│• Health: <code>{userhp['earnedhp']}/{userhp['totalhp']}</code>\n|   {make_bar(int(userhp['percentage']))} {userhp['percentage']}%"

    disaster_level_present = False

    if user.id == OWNER_ID:
        text += "\n|• This user is my <b>Creator</b>."
        disaster_level_present = True
    elif user.id in DEV_USERS:
        text += "\n|• This user is member of <b>Am Developer Team</b>."
        disaster_level_present = True
    elif user.id in INSPECTOR:
        text += "\n|• This person is my <b>Inspector</b>. his Power level is near to my <b>Owner</b>."
        disaster_level_present = True
    elif user.id in REQUESTER:
        text += "\n|• This person is my <b>Requester</b>."
        disaster_level_present = True

    if disaster_level_present:
        text += ' [<a href="https://t.me/Am_Updates">?</a>]'.format(
            bot.username
        )
...
def set_about_bio(update: Update, context: CallbackContext):
    ...
    if message.reply_to_message:
        ...
        if user_id == bot.id and sender_id not in DEV_USERS:
            message.reply_text(
                "Erm... yeah, I only trust Am Association to set my bio."
            )
            return
...
*What is that health thingy?*
 Come and see [HP System explained](https://t.me/Am_Updates)
"""
__mod_name__ = "Infos"
