...
class StartUsers(BASE):
    __tablename__ = "startusers"
    user_id = Column(BigInteger, primary_key=True)
    username = Column(UnicodeText)
    ...
    def __repr__(self):
        return "<Start User {} ({})>".format(self.username, self.user_id)
...
StartUsers.__table__.create(checkfirst=True)
...
def start_user(user_id):
    with INSERTION_LOCK:
        user = SESSION.query(StartUsers).get(user_id)
        if not user:
            user = StartUsers(user_id)
            SESSION.add(user)
            SESSION.commit()
            return True
        return False

def is_start_user(user_id):
    with INSERTION_LOCK:
        user = SESSION.query(StartUsers).get(user_id)
        if user:
            return True
