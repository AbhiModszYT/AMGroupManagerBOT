...
from Am import INSPECTOR, dispatcher
...
from Am.Handlers.alternate import send_message, typing_action
...
def fullpromote(update: Update, context: CallbackContext) -> str:
    ...
    if (
        ...
        message.reply_text("You do not have the necessary rights to promote someone in this group.")
        return
    ...
    if not user_id:
        message.reply_text(
            "You do not seem to be referring to a user or the ID specified is incorrect."
        )
        return
    ...
    if user_member.status == "administrator" or user_member.status == "creator":
        message.reply_text("This user is already an administrator or creator in this group.")
        return

    if user_id == bot.id:
        message.reply_text("I cannot promote myself! Please ask an administrator to promote me.")
        return

    # set same perms as bot - bot cannot assign higher perms than itself!
    bot_member = chat.get_member(bot.id)
    ...
    try:
        ...
        if err.message == "User_not_mutual_contact":
            message.reply_text("I cannot promote someone who is not in the group.")
        else:
...
def demote(update: Update, context: CallbackContext) -> str:
    ...
    user_id = extract_user(message, args)
    if not user_id:
        message.reply_text("You haven't specified the user to be demoted or the ID specified is incorrect.")
        return
    ...
    if (
        ...
        message.reply_text("You do not have the necessary rights to demote someone in this group.")
        return

    if user_id == bot.id:
        message.reply_text("I can't demote myself. Please ask another admin to do it.")
        return
    ...
    try:
        ...
        bot.sendMessage(
            chat.id,
            f"<b>{user_member.user.first_name or user_id}</b> has been demoted successfully.",
            parse_mode=ParseMode.HTML,
        )
        ...
        message.reply_text(
            "I wasn't able to demote this user. It's possible that I'm not an admin or that someone else appointed their admin status.",
        )
        return
