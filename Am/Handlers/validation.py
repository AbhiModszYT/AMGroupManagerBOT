...
from Am import INSPECTOR, REQUESTER, DEV_USERS, DEL_CMDS, SUPPORT_CHAT, dispatcher
from threading import RLock
from cachetools import TTLCache
from time import perf_counter
from functools import wraps
from telegram.ext import CallbackContext
from Am.Database.mod_sql import is_modd
from Am.Database.users_sql import is_start_user
...
def dev_plus(func):
    ...
    def is_dev_plus_func(update: Update, context: CallbackContext, *args, **kwargs):
        ...
        if user.id in DEV_USERS:
           ...
           query.answer(text="This is a developer restricted command.",
            show_alert=True)
...
def validation_of_user(func):
    @wraps(func)
    def is_user_in_db(update: Update, context: CallbackContext, *args, **kwargs):
        user_id = update.effective_user.id

        if update.effective_chat.type != "private":
            check_user = is_start_user(user_id)
            if not check_user:
                # User is trying to use command outside of DM
                message_text = "To use this command in a group chat, you need to start the bot first by sending the /start command in a direct message. Once you have started the bot, you can then use this command in any group chat."
                button_text = "Start"
                bot_username = context.bot.username
                start_command = "start"
                button_url = f"https://t.me/{bot_username}?start={start_command}&param1=value1"
                button = InlineKeyboardButton(text=button_text, url=button_url)
                keyboard = InlineKeyboardMarkup([[button]])
                update.message.reply_text(text=message_text, reply_markup=keyboard)
...
def user_admin(func):
    ...
    def is_admin(update: Update, context: CallbackContext, *args, **kwargs):
        ...
        if user and is_user_admin(chat, user.id):
            ...
            try:
                bot.send_message(user.id, "You're not part of group staff, you cannot use admins command." )
...
from Am.Plugins.Admin import connection
