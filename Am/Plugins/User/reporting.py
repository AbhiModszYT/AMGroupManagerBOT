...
from Am import INSPECTOR, LOGGER, REQUESTER, dispatcher
from Am.Handlers.validation import user_admin, bot_admin, is_user_admin
from Am.Plugins.Admin.log_channel import loggable
from Am.Database import reporting_sql as sql
    ...
    if chat.type == chat.PRIVATE:
        if len(args) >= 1:
            if args[0] in ("yes", "on"):
                ...
                msg.reply_text(
                    "Turned on reporting! You'll be notified whenever anAm reports something."
                )
...
REPORT_BUTTON2_USER_HANDLER = CallbackQueryHandler(rm_button, pattern=r"rm_report", run_async=True)
dispatcher.add_handler(REPORT_BUTTON_USER_HANDLER)
dispatcher.add_handler(REPORT_BUTTON2_USER_HANDLER)
